import os
goods_list = ['กระเพราหมูกรอบ','กระเพราไก่','ผัดเผ็ดปลาดุก','ส้มตำ']
price_list = [40,50,40,70]
class market :
    def list_def(self):
        for x in range(0,len(goods_list)):
            print(x+1,goods_list[x],price_list[x],'บาท')
    def choose(self):
        print('*********อิ่มใจอาหารตามสั่ง*********')
    ...
    def add_list(self):
        while True:
            print('เพิ่มรายการสินค้า หากต้องการออกจากโปรแกรมกด exit')
            add_name = input('เพิ่มชื่อสินค้า :')
            if add_name == 'exit':
                break
            else : 
                add_price = input('เพิ่มราคาสินค้า :')
                goods_list.append(add_name)
